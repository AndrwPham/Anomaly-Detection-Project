...
# extract objects with given coords in tlbr
def extract_objects(frame, detections_coords) -> List:
    '''
    Args:
        detection_coords: list of coordinates in tlbr format, each item is a coord for each detection
    '''
    extracted_objects = []
    for coord in detections_coords:
        x1, y1, x2, y2 = map(int, coord)
        detected_object = frame[x1:x2, y1:y2]
        extracted_objects.append(detected_object)
